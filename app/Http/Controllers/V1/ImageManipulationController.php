...
class ImageManipulationController extends Controller
{
    ...
    public function resize(ResizeImageRequest $request)
    {
        ...
        $absoutePath = public_path($dir);
        File::makeDirectory($absoutePath);

        if($image instanceof UploadedFile){
            $data['name'] = $image->getClientOriginalName();
            // chg file name test.png -> test-resize.png
            $filename = pathinfo($data['name'], PATHINFO_FILENAME);
            $extension = $image->getClientOriginalExtension();

            $image->move($absoutePath,$data['name']);
            ...
            $originalPath = $absoutePath.$data['name'];
        ...
        $image->resize($width, $height)->save($absoutePath.$resizedFilename);
    ...
    public function getImageWidthAndHeight($w,$h,string $originalPath){
